...
public class UnBanCmd implements Command {
    ...
    public void execute(CommandSource sender, String @NonNull [] args) {
        ...
        if (args.length < 1) {
            TextComponentBuilder.createTextComponent(TextUtils.getPrefix("MODERATION"))
                    .appendNewComponentBuilder("Syntax: /unban <pseudo>").setColor(Color.RED)
                    .sendTo(player.getUniqueId());
            return;
        }
        ...
        if (apiPlayerTarget.unBan(APIPlayerModerator)) {
            TextComponentBuilder.createTextComponent(TextUtils.getPrefix("MODERATION"))
                    .appendNewComponentBuilder("La personne se nommant: " + apiPlayerTarget.getName() + " est maintenant unBan.").setColor(Color.GREEN)
                    .sendTo(player.getUniqueId());
        } else {
            TextComponentBuilder.createTextComponent(TextUtils.getPrefix("MODERATION"))
                    .appendNewComponentBuilder("Impossible de unban: " + apiPlayerTarget.getName()).setColor(Color.RED)
                    .sendTo(player.getUniqueId());
        }
