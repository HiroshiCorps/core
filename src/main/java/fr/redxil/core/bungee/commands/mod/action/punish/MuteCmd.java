...
public class MuteCmd implements Command {
    ...
    public void execute(CommandSource sender, String @NonNull [] args) {
        ...
        if (args.length < 3) {
            TextComponentBuilder.createTextComponent(TextUtils.getPrefix("MODERATION"))
                    .appendNewComponentBuilder("Syntax: /mute <pseudo> <temps> <raison>").setColor(Color.RED)
                    .sendTo(player.getUniqueId());
            return;
        }
        ...
        if (apiPlayerTarget.isMute()) {
            TextComponentBuilder.createTextComponent(TextUtils.getPrefix("MODERATION"))
                    .appendNewComponentBuilder("Erreur, le joueur est déjà mute.").setColor(Color.RED)
                    .sendTo(player.getUniqueId());
            return;
        }
