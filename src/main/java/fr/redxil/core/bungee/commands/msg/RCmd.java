    ...
    public void execute(CommandSource sender, String @NonNull [] args) {
        ...
        if (args.length < 1) {
            TextComponentBuilder.createTextComponent(TextUtils.getPrefix("mp"))
                    .appendNewComponentBuilder("Merci de faire /r (message)").setColor(Color.RED).sendTo(playerUUID);
            return;
        }
        ...
        if (targetName == null) {
            TextComponentBuilder.createTextComponent(TextUtils.getPrefix("mp"))
                    .appendNewComponentBuilder("Erreur, vous avez jusque la pas envoyé de message").setColor(Color.RED).sendTo(playerUUID);
            return;
        }
        ...
        if (target == null) {
            TextComponentBuilder.createTextComponent(TextUtils.getPrefix("mp"))
                    .appendNewComponentBuilder("Le joueur: " + targetName + " n'est pas connecté").setColor(Color.RED).sendTo(playerUUID);
            return;
        }
        ...
        TextComponentBuilder.createTextComponent(TextUtils.getPrefix("mp"))
                .appendNewComponentBuilder(targetName).setColor(Color.RED)
                .appendNewComponentBuilder(": ").setColor(Color.WHITE)
